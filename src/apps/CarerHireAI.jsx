...
function ChatInterface({ carer, onBack }) {
  ...
  const sendMessage = async (text) => {
    ...
    // Use ref for always-fresh history
    ...
    try {
      ...
      if (voiceRef.current) {
        ...
        speak(reply, () => {
          setAiSpeaking(false);
          if (voiceRef.current) setTimeout(() => startListening(), 400);
        });
      }
    } catch {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0d0d1a", fontFamily: "Georgia, serif", color: "#fff" }}>
      ...
      <div style={{ borderTop: "1px solid #1e1e35", padding: "28px 24px", textAlign: "center" }}>
        <div style={{ color: "#444", fontSize: 13 }}>🌿 Carer Hire AI • MM AI Technologies • memory-mirror.app</div>
      </div>
    </div>
  );
}
